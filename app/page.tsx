...
export default function Home() {
  ...
  const handleAnalyzeWithAI = async () => {
    if (!result) {
      setError('Primero debes ejecutar una conciliación');
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        const errorMessage = errorData.error || 'Error al procesar el análisis de IA';
        throw new Error(errorMessage);
      }
        ...
        <div className="grid lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
          ...
          {result && (
            <>
              ...
              {/* Analyze with AI Button - Only show when files are uploaded and result exists */}
              {csvFile && excelFile && result && !result.aiAnalysis && (
                <div className="mb-8 flex justify-center">
                  <button
